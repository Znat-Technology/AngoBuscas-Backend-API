...
exports.post = async (req, res, next) => {
    try {
       const { onwner, province, category, institute, county, street, typePlace , description} = req.body
       const images = []
       ...
       const document = new Document({ onwner, depositedPlace,  description, category, images })
 ...
 exports.documentsReceived = async(req, res, next) => {
    ...
    try {
      const response = await Document.findByIdAndUpdate(_id,  {$set: {received: true }})
 ...
 exports.allAccepted =  async (req, res, next) => {
  ...
  try {
    listId.forEach(async (id) => {
      let response = await Document.updateOne({_id: id}, {
        accepted: true
      })
    })
    res.json({ok: true})
